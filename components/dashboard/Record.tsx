...
const getStatusIconAndMessage = (status: string) => {
  switch (status) {
    case RecordStatus.UPLOADED_TO_S3:
      return { icon: <FaClock className="text-orange-500" />, message: "Queued", tooltip: "Uploaded to AWS S3 and is queued for further processing to IPFS and EVM." };
    case RecordStatus.UPLOADED_TO_IPFS:
      return { icon: <FaClock className="text-yellow-500" />, message: "Queued", tooltip: "Uploaded to IPFS and is queued for further processing to EVM." };
    case RecordStatus.UPLOADED_TO_EVM:
      return { icon: <FaCheckCircle className="text-green-500" />, message: "Uploaded", tooltip: "Successfully uploaded to the Base blockchain." };
    default:
        ...
        <table className="table-auto w-full">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 text-xs md:text-sm lg:text-base">Progress</th>
            ...
            {records.map((record) => {
              ...
              return (
                <tr key={record.id} className="border-t">
                  ...
                  <td className="px-4 py-2 text-xs md:text-sm lg:text-base text-center">{progress}</td>
                  <td className="px-4 py-2 text-xs md:text-sm lg:text-base text-center">
                    <button
